...
public class FunctionalityTest {
    ...
    @Test
    /**
     * Animal name is one word, species remains unchanged as specified and
     * treatments arraylist updated with feeding and cage cleaning
     */
    ...
    @Test
    /**
     * Correct animal input test, animal has two names so checks for species change
     * to kits and correct treatments
    ...
    public void TestAnimalTwonames() {
        ...
        assertEquals(
                "When an animal has two words in its name or two names it should be changed to the animaltype KIT species",
                expectedanimal, animal.getSpecies());
    ...
    @Test(expected = IllegalArgumentException.class)
    /**
     * Test to ensure an IllegalArgumentException is thrown for an invalid species
    ...
    @Test
    // Test to ensure the animal constructor correctly initializes the animal object
    // when the ID is set to 0
    ...
    // Test to ensure a NullPointerException is thrown for null species input in the
    // constructor
    ...
    @Test
    /**
     * Testing addTreatments method with correct input and matching ID that
     * satisfies "if" expression
     */
    ...
    public void testAddTreatmentsWithInvalidAnimalID() {
        ...
        /*
         * We expect 2 treatments to be in AnimalTreatments, the treatments that
         * populate by way of the Animal constructor, meaning the above treatments were
         * not added
         */
        assertEquals("Treatments were added to animal's treatment list even though ID did not match!", 2,
                (rightanimal.getAnimalTreatments()).size());
    }

    @Test
    /**
     * Test method addTreatments of the Animal class by adding more than 2,
    ...
    public void testAddMultipleTreatments() {
        ...
        for (Treatments treat : treatments) {
            assertTrue("Failed to add expected treatment to animal's list of treatments",
                    rightanimal.getAnimalTreatments().contains(treat));
        }
    }
    ...
    public void testAddTreatmentsForMultipleAnimalsTwoTreatmentsEach() {
        ...
        // expected is 4 for both because new animal object will cause constructor
        // "Animal" to populate AnimalTreatments with 2 treatments first, then the other
        // 2 treatments we add here makes 4
    ...
    public void testAddTreatmentsToAnimalWithNoExistingTreatments() {
        ...
        rightanimal.addTreatments(treatments);
        assertEquals(
                "Adding an empty treatments list to an animal with existing treatments should not change the number of treatments",
                2, rightanimal.getAnimalTreatments().size());
    ...
    public void testGetSortedTreatmentsEmptyAnimals() {
        ...
        assertTrue("The treatments list should be empty when there are no animals in the schedule",
                treatments.isEmpty());
    }
    ...
    public void testGetSortedTreatmentsOneAnimalOneTreatment() {
        ...
        ArrayList<Treatments> sortedTreatments = schedule.getSortedTreatments();
        assertEquals("Expected the size of sortedTreatments ArrayList to be 3", treatment1.getMaxWindow(),
                sortedTreatments.size());
    }
    ...
    public void testGetSortedTreatmentsManyAnimalsManyTreatments() {
        ...
        // Sort the treatments
        ArrayList<Treatments> sortedTreatments = schedule.getSortedTreatments();
        assertEquals("The size of the sorted treatments list should match the size of the expected treatments list",
                sortedTreatments.size(), testTreatments.size());

        // Check if the sorted list is sorted correctly, ascending or descending order
        // based on what is set in the schedule class
        for (int i = 0; i < testTreatments.size(); i++) {
            Treatments exp = sortedTreatments.get(i);
            Treatments last = sortedTreatments.get(testTreatments.size() - 1);
            assertTrue(
                    "The animal ID of the current treatment in the sorted list should be less than or equal to the last animal ID",
                    exp.getAnimalID() <= last.getAnimalID());
            assertTrue(
                    "The duration of the current treatment in the sorted list should be greater than or equal to the last duration",
                    exp.getDuration() >= last.getDuration());
            assertTrue(
                    "The setup time of the current treatment in the sorted list should be greater than or equal to the last setup time",
                    exp.getSetupTime() >= last.getSetupTime());
            assertTrue(
                    "The start hour of the current treatment in the sorted list should be greater than or equal to the last start hour",
                    exp.getStartHour() >= last.getStartHour());
            assertTrue(
                    "The max window of the current treatment in the sorted list should be less than or equal to the last max window",
                    exp.getMaxWindow() <= last.getMaxWindow());
    ...
    public void testGetSortedTreatmentsSameAnimalTwiceManyTreatments() {
        ...
        for (int i = 0; i < testTreatments.size(); i++) {
            ...
            assertTrue(
                    ...
                    exp.getAnimalID() <= last.getAnimalID());
            assertTrue(
                    "The duration of the current treatment in the sorted list should be less than or equal to the last duration",
                    exp.getDuration() <= last.getDuration());
            assertTrue(
                    "The setup time of the current treatment in the sorted list should be greater than or equal to the last setup time",
                    exp.getSetupTime() >= last.getSetupTime());
    ...
    @Test
    /**
     * This test checks if the createSchedule method initializes the schedule
     * correctly when no treatments are added
     */
    public void testCreateScheduleNoTreatments() throws ScheduleOverflowException {
        ...
        assertEquals(
                "Each row in the schedule array should have 12 columns, one for each 5-minute interval within an hour",
                12, scheduleArray[0].length);
    }
    ...
    public void testCreateScheduleAddsManyTreatments() throws ScheduleOverflowException {
        ...
        assertEquals("The expected number of treatments were not added to the schedule", expectedScheduleSize,
                actualScheduleSize);
    }

    @Test
    /**
     * This test checks if the createSchedule method properly handles Kit feeding
     * treatments
     */
    public void testCreateScheduleHoldsKitFeeding() throws ScheduleOverflowException {
        // test that Kit feeding treatment is held separately and added to the schedule
        // later
        ...
        schedule.createSchedule();
        Treatments[][] scheduleArray = schedule.getSchedule();
        assertEquals("Kit feeding was not added or not added correctly to the schedule",
                testTreatments.get(0).getDescription(), scheduleArray[0][0].getDescription());
        assertEquals("Kit feeding start hour was not added or not added correctly to the schedule",
                testTreatments.get(0).getStartHour(), scheduleArray[0][0].getStartHour());
    }
    ...
    public void testWriteScheduleNoTreatments() throws IOException, ScheduleOverflowException {
        ...
        String scheduleString = new String(Files.readAllBytes(Paths.get(String.format("%s.txt", tomorrow))),
    ...
    @Test
    /**
     * Tests writeSchedule with one extra feeding treatment added into schedule
     * other than the initialized treatments
     */
    public void testWriteScheduleOneTreatment() throws IOException, ScheduleOverflowException {
        ...
        assertTrue("Schedule string should contain the animal's nickname",
                scheduleString.contains(rightanimal.getNickname()));
        assertTrue("Schedule string should contain the feeding treatment for the animal",
                scheduleString.contains(String.format("Feeding - %s", animalNameOne)));
        assertTrue("Schedule string should contain the cleaning treatment for the animal's cage",
                scheduleString.contains(String.format("Clean %s Cage",
                        rightanimal.getSpecies().toString().toLowerCase())));
        assertFalse("Schedule string should not contain backup volunteer treatment",
                scheduleString.contains("+ backup volunteer"));
     ...
     * schedule should have just the default treatments and no backup volunteer is
     * called in
    ...
    public void testWriteScheduleTwoSAnimals() throws IOException, ScheduleOverflowException {
        ...
        assertTrue("Schedule string should contain the nickname of the first animal",
                scheduleString.contains(rightanimal.getNickname()));
        assertTrue("Schedule string should contain the nickname of the second animal",
                scheduleString.contains(animal2.getNickname()));
        assertFalse("Feeding treatment for the first animal should not be included in the schedule",
                scheduleString.contains(String.format("Feeding - %s", animalSpeciesOne)));
        assertTrue("Feeding treatment for the second animal should be included in the schedule",
                scheduleString.contains(String.format("Feeding - %s", animalSpeciesTwo)));
        assertTrue("Clean cage treatment for the first animal should be included in the schedule",
                scheduleString.contains(String.format("Clean %s Cage", animalSpeciesOne)));
        assertTrue("Clean cage treatment for the second animal should be included in the schedule",
                scheduleString.contains(String.format("Clean %s Cage", animalSpeciesTwo)));
        assertFalse("Backup volunteer should not be called in for the schedule",
                scheduleString.contains("+ backup volunteer"));
    ...
    @Test
    /**
     * Tests writing schedule with more than one animal and many treatments, some
     * unusual treatment types
     * As long as times are distinct, all treatments should show up and are tested
     * by description
    ...
    public void testWriteScheduleTwoSAnimalsManyTreatmentsAdded() throws IOException, ScheduleOverflowException {
        ...
        assertTrue("Schedule string should contain the first animal's name",
                scheduleString.contains(rightanimal.getNickname()));
        assertTrue("Schedule string should contain the second animal's name",
                scheduleString.contains(animal2.getNickname()));
        // Check for specific descriptions which will be displayed from the treatments,
        // this works for specific spread out times as the schedule tries to optimize
        // depending on startHour
        assertTrue("Schedule string should contain the first animal's feeding treatment",
                scheduleString.contains(String.format("Feeding - %s", animalSpeciesOne)));
        assertTrue("Schedule string should contain the second animal's feeding treatment",
                scheduleString.contains(String.format("Feeding - %s", animalSpeciesTwo)));
        assertTrue("Schedule string should contain the first animal's cage cleaning treatment",
                scheduleString.contains(String.format("Clean %s Cage", animalSpeciesOne)));
        assertTrue("Schedule string should contain the second animal's cage cleaning treatment",
                scheduleString.contains(String.format("Clean %s Cage", animalSpeciesTwo)));
        assertTrue("Schedule string should contain the first animal's playtime treatment",
                scheduleString.contains(String.format("Play with %s", animalNameOne)));
        assertFalse("Schedule string should not contain backup volunteer",
                scheduleString.contains("+ backup volunteer"));
    ...
    /**
     * Tests that writeSchedule will throw IOexception when given an invalid file
     * name
     */
    public void testWriteScheduleIOException() throws IOException, ScheduleOverflowException {
        ...
        String scheduleString = new String(Files.readAllBytes(Paths.get(String.format("%HELLO.txt", tomorrow))),
                StandardCharsets.UTF_8);
        // Test that schedule string is empty when there are no treatments scheduled
    ...
    @Test(expected = ScheduleOverflowException.class)
    /**
     * Test checks if checks ScheduleOverflowException is thrown when the schedule
     * file runs out of space
     */
    ...
    @Test(expected = ScheduleOverflowException.class)
    /**
     * Test checks if checks ScheduleOverflowException is thrown when the
     * writeschedule is presented with treatments that have invalid startHours
     */
    public void testWriteScheduleScheduleOverflowExceptionInvalidStartHour()
            throws IOException, ScheduleOverflowException {
        LocalDate tomorrow = LocalDate.now().plusDays(1);
        animalsArray.add(rightanimal);
        ArrayList<Treatments> treatments = new ArrayList<>();
    ...
    public void testWriteScheduleWithKits() throws IOException, ScheduleOverflowException {
        ...
        // read the schedule file and verify that it includes treatments for both
        // animals
        String scheduleString = new String(
                Files.readAllBytes(Paths.get(String.format("%s.txt", LocalDate.now().plusDays(1)))),
                StandardCharsets.UTF_8);

        assertTrue("Schedule should include treatments for adult animal",
                scheduleString.contains(rightanimal.getNickname()));
        assertTrue("Schedule should include treatments for kit", scheduleString.contains(kit.getNickname()));
        assertTrue("Schedule should include cleaning treatment for kit's cage",
                scheduleString.contains(String.format("Clean kits Cage (%s)", kit.getNickname())));
        assertTrue("Schedule should include cleaning treatment for adult animal's cage",
                scheduleString.contains(String.format("Clean %s Cage (%s)",
                        adultSpecies, rightanimal.getNickname())));
        assertTrue("Schedule should include feeding treatment for adult animal",
                scheduleString.contains(String.format("Feeding - %s", adultSpecies)));
    ...
    @Test
    /**
     * Tests that the schedule doesn't include a backup volunteer when there are
     * many duplicate treatments,
    ...
    public void testWriteScheduleNoBackupVolunteerDuplicateTreatments() throws IOException, ScheduleOverflowException {
        ...
        assertTrue("Schedule should contain a treatment for cleaning the animal's cage",
                scheduleString.contains(String.format("Clean %s Cage (%s)",
                        rightanimal.getSpecies().toString().toLowerCase(), rightanimal.getNickname())));
        assertTrue("Schedule should contain a treatment for feeding the animal",
                scheduleString.contains(String.format("Feeding - %s",
                        rightanimal.getSpecies().toString().toLowerCase())));
        assertFalse("Schedule should not contain a backup volunteer when there are duplicate treatments",
                scheduleString.contains("+ backup volunteer"));
    ...
    @Test
    /**
     * Tests that the schedule includes a backup volunteer when no more treatments
     * can be added and no backup treatments are allowed
     */
    public void testWriteScheduleWithBackupVolunteer() throws IOException, ScheduleOverflowException {
        ...
        assertTrue("Schedule should include nickname of the animal",
                scheduleString.contains(rightanimal.getNickname()));
        assertTrue("Schedule should include cleaning task for the animal's cage", scheduleString
                .contains(String.format("Clean %s Cage", rightanimal.getSpecies().toString().toLowerCase())));
        assertTrue("Schedule should include feeding task for the animal", scheduleString
                .contains(String.format("Feeding - %s", rightanimal.getSpecies().toString().toLowerCase())));
        assertTrue(
                "Schedule should include a backup volunteer when no more treatments can be added and no backup treatments are allowed",
                scheduleString.contains("+ backup volunteer"));
